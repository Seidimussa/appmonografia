document.addEventListener('DOMContentLoaded', () => {
    ...
    const TOTAL_MODULES = 6; // Atualizado para 6 módulos de SQL

    // Usaremos localStorage para persistir o progresso
    let moduleState = JSON.parse(localStorage.getItem('sqlCourseProgress')) || { // Alterado para 'sqlCourseProgress'
        'sql-basico': { percentage: 0, completed: false },
        'gestao-tabelas': { percentage: 0, completed: false },
        'filtros': { percentage: 0, completed: false },
        'funcoes-agregadas': { percentage: 0, completed: false },
        'joins': { percentage: 0, completed: false },
        'subconsultas': { percentage: 0, completed: false }
    };

    function saveProgress() {
        localStorage.setItem('sqlCourseProgress', JSON.stringify(moduleState));
    }
    ...
    function updateModuleUI() {
        ...
        moduleCards.forEach(card => {
            ...
            if (moduleName === 'sql-basico') {
                card.classList.remove('locked');
                card.classList.add('active');
                if (lockIcon) lockIcon.remove();
            }
            // Os módulos seguintes são desbloqueados se o anterior estiver 100%
            else if (moduleName === 'gestao-tabelas' && moduleState['sql-basico'].percentage === 100) {
                card.classList.remove('locked');
                card.classList.add('active');
                if (lockIcon) lockIcon.remove();
            } else if (moduleName === 'filtros' && moduleState['gestao-tabelas'].percentage === 100) {
                card.classList.remove('locked');
                card.classList.add('active');
                if (lockIcon) lockIcon.remove();
            } else if (moduleName === 'funcoes-agregadas' && moduleState.filtros.percentage === 100) {
                card.classList.remove('locked');
                card.classList.add('active');
                if (lockIcon) lockIcon.remove();
            } else if (moduleName === 'joins' && moduleState['funcoes-agregadas'].percentage === 100) {
                card.classList.remove('locked');
                card.classList.add('active');
                if (lockIcon) lockIcon.remove();
            } else if (moduleName === 'subconsultas' && moduleState.joins.percentage === 100) {
                card.classList.remove('locked');
                card.classList.add('active');
                if (lockIcon) lockIcon.remove();
            }
